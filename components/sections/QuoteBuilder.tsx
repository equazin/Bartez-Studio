...
export function QuoteBuilder() {
  ...
  const toggle = (c: (typeof quote.categories)[number]) =>
    setSel((s) => {
      ...
      else next[c.id] = { qty: 1, variant: c.variants?.[0] };
      return next;
    });
  ...
  return (
    <section id="cotiza" className="scroll-mt-24 bg-slate-50 py-24">
      <div className="mx-auto max-w-[1200px] px-7">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-soft md:p-9">
            ...
            {step === 1 && (
              <div>
                <h3 className="mb-4 font-display text-[18px] font-bold text-ink">Cantidades y detalle</h3>
                <div className="space-y-3">
                  {selectedCats.map((c) => (
                    <div key={c.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 p-3.5">
                      ...
                      {c.variants && (
                        <select
                          ...
                          className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-[13px] focus:border-brand focus:outline-none"
                        >
                          {c.variants.map((v) => (
      ...
      .qb-input:focus{outline:none;border-color:#1d4ed8;box-shadow:0 0 0 3px rgba(29,78,216,.15)}`}</style>
    </section>
  );
}

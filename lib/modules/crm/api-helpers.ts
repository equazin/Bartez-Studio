...
/**
 * Boilerplate común para todas las API routes del CRM:
 *  - autoriza la sesión
 *  - resuelve el organizationId del tenant (con fallback a org semilla)
 *  - aplica rate limit a mutaciones
 *  - chequea el permiso RBAC
 *
 * Devuelve `{ ok: false, response }` con la respuesta lista para retornar,
 * o `{ ok: true, session, orgId }` para continuar el handler.
 */
...
> {
  ...
  if (options.mutation) {
    const scope = options.rateLimitScope ?? "admin:crm";
...
/** Paginación común a todas las APIs CRM. */

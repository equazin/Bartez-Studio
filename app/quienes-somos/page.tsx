        ...
        <section className="bg-ink py-20 text-white md:py-28">
          <div className="mx-auto max-w-[1200px] px-6">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 py-1.5 mb-6">
              ...
              <span className="text-[12px] font-semibold tracking-wide text-slate-200">Distribuidora IT · Rosario, Argentina</span>
            </div>
            <h1 className="max-w-[700px] font-display text-[clamp(40px,6vw,70px)] font-bold leading-[0.98] tracking-[-0.05em] text-balance">
              ...
              <span className="text-gradient">desde 2008.</span>
            </h1>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-x-10 gap-y-5 border-t border-white/15 pt-8">
              <div>
                ...
                <span className="mt-1 block text-[12px] text-slate-400">años de experiencia</span>
              </div>
              <div>
                <span className="block font-display text-[42px] font-bold leading-none text-accent">2008</span>
                <span className="mt-1 block text-[12px] text-slate-400">año de fundación</span>
              </div>
              <div>
                <span className="block font-display text-[42px] font-bold leading-none text-accent">4</span>
                <span className="mt-1 block text-[12px] text-slate-400">canales atendidos</span>
              </div>
              <div>
                <span className="block font-display text-[42px] font-bold leading-none text-accent">ARG</span>
                <span className="mt-1 block text-[12px] text-slate-400">cobertura nacional</span>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-[1200px] px-6">
            <div className="grid gap-16 lg:grid-cols-2 lg:gap-24">
              <div>
                ...
                <Link
                  href="/#cotiza"
                  className="mt-8 inline-flex items-center gap-2 rounded-lg bg-ink px-6 py-3.5 text-[14.5px] font-semibold text-white transition-colors hover:bg-brand"
                >
                ...
                {values.map((v) => (
                  <div key={v.title} className="flex gap-4 rounded-2xl border border-slate-200 p-5">
                    ...
                    <div>
                      ...
                      <p className="mt-1.5 text-[13.5px] leading-relaxed text-slate-500">{v.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section className="bg-slate-50 py-20 md:py-28">
          <div className="mx-auto max-w-[1200px] px-6">
            ...
            <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {channels.map((ch) => (
                <div key={ch.title} className="rounded-2xl border border-slate-200 bg-white p-6">
                  <span className="grid size-12 place-items-center rounded-xl bg-brand/8">
                    <ch.icon className="size-6 text-brand" strokeWidth={1.5} />
                  </span>
                  <h3 className="mt-5 font-display text-[16px] font-bold text-ink">{ch.title}</h3>
                  <p className="mt-2 text-[13px] leading-relaxed text-slate-500">{ch.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="bg-white py-16">
          <div className="mx-auto max-w-[1200px] px-6 text-center">
            <div className="inline-flex items-center gap-2 mb-4">
              ...
              <span className="text-[13px] font-semibold text-slate-500">Rosario, Santa Fe · Cobertura nacional</span>
            </div>
            <h2 className="font-display text-[clamp(26px,3.5vw,40px)] font-bold tracking-[-0.035em] text-ink">
              ¿Querés trabajar con nosotros?
            </h2>
            <p className="mx-auto mt-4 max-w-[50ch] text-[15px] leading-relaxed text-slate-600">
              Contanos tu necesidad y un especialista te contactará con una propuesta concreta en 24 hs hábiles.
            </p>
            <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Link href="/#cotiza" className="inline-flex items-center gap-2 rounded-lg bg-ink px-6 py-3.5 text-[14.5px] font-semibold text-white transition-colors hover:bg-brand">
                Contanos qué necesitás <ArrowRight size={16} />
              </Link>
              <Link href="/revendedores" className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-6 py-3.5 text-[14.5px] font-semibold text-ink transition-colors hover:border-brand hover:text-brand">
